...
for i in range(num_envs):
    ...
    for j in range(len(loaded_assets)):
        pose.p = gymapi.Vec3(0.0, j*10, height-j*30)
    ...
    #                                 gym.get_rigid_handle(env, gym.get_actor_name(env, actor_handle), gym.get_actor_rigid_body_names(env, actor_handle)[0]),
    #                                 gymapi.Vec3(0., 20., 0.))
...
# print("##### rb_states : ", rb_states, ' ',rb_states.shape)

rb_positions = rb_states[:, 0:3].view(num_envs, num_bodies*2, 3)
...
while not gym.query_viewer_has_closed(viewer):
    ...
    for i in range(num_envs):
        ...
        gym.set_rigid_linear_velocity(envs[i],
                                    gym.get_rigid_handle(envs[i], gym.get_actor_name(envs[i], actor_handles[i][0]), gym.get_actor_rigid_body_names(envs[i], actor_handles[i][0])[0]),
                                    gymapi.Vec3(1., 1., 1.1))
