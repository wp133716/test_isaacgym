...
from common.controller3 import cclvf2 as cal_vel
from common.controller3 import cclvf

from common.controller3 import euler2quat, quat2euler, CameraController
from common.controller3 import euler2quaternion, quaternion2euler
...
for i in range(num_envs):
    ...
    local_transform.r = gymapi.Quat.from_euler_zyx(0, 0.5*np.pi, 0)
...
# _ar_states = gym.acquire_actor_root_state_tensor(sim)
# ar_states = gymtorch.wrap_tensor(_ar_states)
...
# actor_pos_batch = ar_states[:, :3].view(num_envs, actor_count, 3)
# uav_pos_batch = ar_states[::2, :3].view(num_envs, 3)
# car_pos_batch = ar_states[1::2, :3].view(num_envs, 3)

# actor_rot_batch = ar_states[:, 3:7].view(num_envs, actor_count, 4)
# uav_rot_batch = ar_states[::2, 3:7].view(num_envs, 4)
# car_rot_batch = ar_states[1::2, 3:7].view(num_envs, 4)

# actor_vel_batch = ar_states[:, 7:10].view(num_envs, actor_count, 3)
# uav_vel_batch = ar_states[::2, 7:10].view(num_envs, 3)
# car_vel_batch = ar_states[1::2, 7:10].view(num_envs, 3)

# actor_ang_vel_batch = ar_states[:, 10:13].view(num_envs, actor_count, 3)
# uav_ang_vel_batch = ar_states[::2, 10:13].view(num_envs, 3)
# car_ang_vel_batch = ar_states[1::2, 10:13].view(num_envs, 3)

# print("############## rb_states :\n", rb_states, ' ', rb_states.shape)
# print("############## ar_states :\n", ar_states, ' ', ar_states.shape)
...
while not gym.query_viewer_has_closed(viewer):
    ...
    car_order_vel = cal_vel(car_pos_batch, target_pos=torch.ones_like(car_pos_batch), speed=10, radius=10)
    # car_order_vel1 = cclvf(car_pos_batch[1], target_pos=[1, 1], speed=10, radius=10)
    # uav_order_vel = cal_vel(uav_pos_batch, target_pos=car_pos_batch, speed=20, radius=20)
    yaw = torch.atan2(car_order_vel[:,1], car_order_vel[:,0])
    # print("~~~~~~~~~~~~~ yaw : ", yaw)
    # print("~~~~~~~~~~~~~ car_order_vel : ", car_order_vel)
    # print("~~~~~~~~~~~~~ car_order_vel1 : ", car_order_vel1)
    euler_buffer = torch.zeros(num_envs, 3)
    euler_buffer[:, 2] = yaw
    # car_quat = euler2quaternion(car_euler_angle)
    # car_quat = gymapi.Quat.from_euler_zyx(0, 0, yaw[0])
    # car_quat = gymapi.Quat.from_buffer(euler_buffer)
    car_quat = euler2quaternion(euler_buffer)

    print("$$$$$$$$$$$$$ car_quat :\n ", car_quat)
    print("$$$$$$$$$$$$$ state_buffer :\n ", state_buffer)
    state_buffer[0::2, 3:7] = torch.tensor([0, 0, 0, 1])
    state_buffer[0::2, 7:10] = torch.tensor([1, 1, 0])
    state_buffer[1::2, 3:7] = torch.tensor(car_quat)
    state_buffer[1::2, 7:10] = car_order_vel
    print(gym.set_actor_root_state_tensor(sim, gymtorch.unwrap_tensor(state_buffer)))
    # print("#########   dof_vel : ", dof_vel)
    # gym.set_dof_velocity_target_tensor(sim, gymtorch.unwrap_tensor(car_order_vel))
    # gym.set_dof_position_target_tensor(sim, gymtorch.unwrap_tensor(targets))
    # gym.set_dof_actuation_force_tensor(sim, gymtorch.unwrap_tensor(force))
    ...
    for i in range(num_envs):
        '''
            ...
            # print("###### actor_pos :\n", actor_pos)
            # print("###### actor_vel :\n", actor_vel)
            # print("ar_states : ", ar_states)
            print("uav_positions : ", uav_positions)
            print("uav_orientations : ", uav_orientations)
            print("uav_vel_linear : ", uav_vel_linear)
            print("uav_vel_angular : ", uav_vel_angular)

            print("car_positions : ", car_positions)
            print("car_orientations : ", car_orientations)
            print("car_vel_linear : ", car_vel_linear)
            print("car_vel_angular : ", car_vel_angular)
        '''
